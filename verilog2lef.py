...
class PHYPortPin(PHYObject):
	...
	def add_rect(self, layer, left_x, bot_y):
		if layer in self.phys_map.keys():
			...
			self.phys_map[layer] = {'coord':[[left_x, bot_y, left_x + self.x_width, bot_y + self.y_width]]}
		...
		for pin_name, pin_info in self.verilog_pin_dict.items():
			...
			if pin_info.get('is_bus', None):
				...
				for pin in range(pin_info['bus_max'] + 1):
					self.pins[pin_name + '_' + str(pin)] = PHYPortPin(pin_info, layer, side, x_width, y_width,
																	  bus_idx=pin)
			else:
				self.n_inputs += direction == 'input'
				self.n_outputs += direction == 'output'
				self.pins[pin_name] = PHYPortPin(pin_info, layer, side, x_width, y_width)
...
class BBoxPHY(PHYDesign):
	...
	def define_design_boundaries(self):
		...
		self.bbox_x_width = x_width
		self.bbox_y_width = y_width
		self.bbox_left_margin = max([pin.x_width for pin in  self.pin_sides_dict['left']], default=0)
		self.bbox_right_margin = max([pin.x_width for pin in self.pin_sides_dict['right']], default=0)
		self.bbox_top_margin = max([pin.y_width for pin in self.pin_sides_dict['top']], default=0)
		self.bbox_bot_margin = max([pin.y_width for pin in self.pin_sides_dict['bottom']], default=0)
		self.x_width = self.bbox_x_width + self.bbox_left_margin + self.bbox_right_margin
		self.y_width = self.bbox_y_width + self.bbox_top_margin + self.bbox_bot_margin
	...
	def build_design_repr(self):
		...
		if self.specs['pin_margin']:
			...
			x_corr = 0
			y_corr = 0
		left_pin_lower_corner = [bbox_bot_left_corner[0], round((self.bbox_bot_margin + y_corr) / h_pin_width)]
		right_pin_lower_corner = [bbox_top_right_corner[0], round((self.bbox_bot_margin + y_corr) / h_pin_width)]
		bot_pin_lower_corner =[round((bbox_bot_left_corner[0] + x_corr) / v_pin_width), 0]
		top_pin_lower_corner =[round((bbox_top_right_corner[0] + x_corr) / v_pin_width), self.y_width]
		...
		for pin in self.pin_sides_dict['left']:
			...
			left_pin_lower_corner[1] = left_pin_lower_corner[1] + h_pin_width + h_pin_pitch
